...
import { Home, Plus, Brush, BarChart3, Wallet, Building2, Wrench, LogOut, Download, Phone, Shield, Users, Database, TrendingUp } from 'lucide-react';
import './style.css';
...
function RoomCard({room,booking,canFinance}){ const unpaid=booking?Math.max(0,Number(booking.total||0)-Number(booking.paid||0)):0; return <Card><div className="row"><div><b>{room.name}</b><p className="muted">{booking?.guest||'No current guest'}</p>{booking?.guest_phone&&<a className="whatsapp" href={whatsappUrl(booking.guest_phone)} target="_blank"><Phone size={14}/> WhatsApp</a>}</div><span className={booking?'pill dark':'pill'}>{booking?'Occupied':'Available'}</span></div><p>Check-in: {formatDate(booking?.check_in)}</p><p>Check-out: {formatDate(booking?.check_out)}</p>{canFinance&&booking&&<p>Unpaid: {money(unpaid)}</p>}</Card> }
function BookingCard({b,canFinance}){ const unpaid=Math.max(0,Number(b.total||0)-Number(b.paid||0)); return <Card><div className="row"><div><b>{b.room_name}</b><p>{b.guest} · {b.source}</p>{b.guest_phone&&<a className="whatsapp" href={whatsappUrl(b.guest_phone)} target="_blank"><Phone size={14}/> WhatsApp</a>}</div><span className="pill">{nights(b)} nights</span></div><p>{formatDate(b.check_in)} → {formatDate(b.check_out)}</p>{canFinance&&<p>Paid: {money(b.paid)} / Total: {money(b.total)} · Unpaid: {money(unpaid)}</p>}</Card> }
